...
class GameMap:
    """Game map with loot zones and extraction points."""
...
class RaidRunner:
    ...
    def run_averaged_raid(self, lobby: List[Player], base_seed: int) -> Dict[int, dict]:
        """Run raid multiple times with different seeds, average results."""
...
def compute_reward(raid_results: List[dict], classifications: Dict[int, str]) -> dict:
    """Compute Pareto reward for a single raid."""
    passive_results = []
    aggressive_results = []
